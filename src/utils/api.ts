...
export function calculateTradePnL(trade: {
  ...
  if (exit_price !== null && !Number.isNaN(exit_price)) {
    const pnl =
      trade.type === 'LONG' ? (exit_price - entry_price) * quantity - fee : (entry_price - exit_price) * quantity - fee;
...
export const api = {
  ...
  async createAccount(
    userId: string,
    accountData: Omit<BrokerAccount, 'id' | 'createdAt' | 'currentCapital'>,
  ...
  async updateTrade(userId: string, accountId: string, tradeId: string, tradeData: Partial<Trade>): Promise<Trade> {
    const db = getFirestoreDb();
  ...
  async importCSV(userId: string, accountId: string, csvText: string): Promise<{ success: boolean; count: number }> {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const cleanExitPrice = Number.isNaN(Number(exit_price)) ? null : exit_price;
      const cleanFee = Number.isNaN(fee) ? 0 : fee;

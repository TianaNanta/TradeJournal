...
- **Data fetching**: `refreshData()` in `App.tsx` calls `api.getAccounts()` and (if activeAccount selected) `api.getTrades()` in parallel. Re-fetched whenever any mutating action completes.
...
|Directory|Purpose|
|---|---|
|`src/`|React frontend source (17 files)|
|`src/components/`|7 UI components — Dashboard, Journal, TradeModal, CalendarView, ImportExport, Auth, FirebaseSetup|
|`src/utils/`|Firebase init (`firebase.ts`), Firestore CRUD (`api.ts`), stats computation (`stats.ts`), currency formatting (`format.ts`), demo data (`demoData.ts`)|
...
- **Strict mode** enabled (`"strict": true`). All `noUnusedLocals`, `noUnusedParameters`, `noUnusedParameters`, `noImplicitReturns` are on.
...
- **Glass-morphism cards**: `bg-brand-card/60 backdrop-blur-md` pattern used across components.
...
- TradeModal performs client-side field validation before submission (required fields, exit date > entry date).
...
- **Firebase lazy init** (`src/utils/firebase.ts`): Supports both `VITE_FIREBASE_*` env vars (build-time) and localStorage config (runtime). Singleton pattern with module-level `let` variables. Exports getter functions (`getFirebaseAuth()`, `getFirestoreDb()`) that initialize on first call via `initFirebase()`. Throws if accessed before initialization.
- **Firestore CRUD** (`src/utils/api.ts`): Singleton `api` object with 8 methods. PnL computed client-side via `calculateTradePnL()`. Account capital updated atomically via Firebase `increment()`. Batch writes for cascade deletes and CSV import (batches of 490 to stay under Firestore limit). Firestore path: `users/{userId}/accounts/{accountId}/trades/{tradeId}`.
- **Stats computation** (`src/utils/stats.ts`): Pure function `calculateStats(trades: Trade[]) → DashboardStats`. No side effects, no Firebase dependency. Computes cumulative PnL, win rate, profit factor, avg win/loss, PnL by setup, win/loss counts.
- **Demo data** (`src/utils/demoData.ts`): Exports `DEMO_TRADES_CSV` (40 demo trades as CSV string) and `importDemoData()` which delegates to `api.importCSV()`.
...
- **Framework**: None. Raw `Bun.serve` with manual string-conditional routing on URL pathname and HTTP method.
- **Database**: `bun:sqlite` with raw SQL queries (no ORM). Database file is `trades.db` at project root (gitignored). Single table `trades`: id, symbol, type, quantity, entry_price, exit_price, entry_date, exit_date, fee, status, pnl, setup, notes, created_at.
- **PnL calculation**: Server-side on create/update:
...
- **API routing**: Raw `fetch` handler on `Bun.serve` — no Express, Elysia, or Hono. Manual route matching on URL pathname. Routes: GET/POST `/api/trades`, GET/PUT/DELETE `/api/trades/:id`, GET `/api/stats`, POST `/api/import`.
- **CSV import**: Server-side parser handles quoted fields, validates headers client-side before sending. Required fields: symbol, type, quantity, entry_price, entry_date. Optional: exit_price, exit_date, fee, setup, notes.
- **CORS**: Manual `OPTIONS` preflight handler. Allowed origin: `http://localhost:3000` (Vite dev server). All JSON responses include CORS headers.
- **Static serving**: For non-`/api` paths, serves files from `dist/` with SPA fallback to `dist/index.html`.
- **Error handling**: All API errors return JSON `{ error: string }`. Invalid JSON/missing fields → 400, not found → 404, unhandled → 500 with console.error.
...
|`src/App.tsx`|Root component (901 lines) — global state, auth, accounts, tabs, all modals|
|`src/types.ts`|`Trade`, `BrokerAccount`, `DashboardStats` interfaces|
|`src/index.css`|Tailwind directives + custom dark scrollbar styles|
...
|`src/utils/demoData.ts`|Demo data CSV string and import function|
...
|`package.json`|Scripts, dependencies — firebase 12, react 18, vite 5, biome 2.5, tailwind 3.4|
...
- **PostCSS**: Tailwind CSS + Autoprefixer configured.

import { initDb, getTrades, getTrade, createTrade, updateTrade, deleteTrade, getStats, type TradeInput } from "./db";
...
function importCSVTrades(csvText: string): number {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    createTrade({
      ...
      quantity,
      entry_price,
      exit_price: isNaN(Number(exit_price)) ? null : exit_price,
      entry_date,
      exit_date,
      fee: isNaN(fee) ? 0 : fee,
      setup,
        ...
        if (tradeIdMatch) {
          ...
          if (method === "PUT") {
            ...
            if (body && typeof body === "object") {
              ...
              if (data.exit_price !== undefined) updateData.exit_price = data.exit_price !== null && data.exit_price !== '' ? Number(data.exit_price) : null;
              if (data.entry_date !== undefined) updateData.entry_date = String(data.entry_date);
              if (data.exit_date !== undefined) updateData.exit_date = data.exit_date !== null && data.exit_date !== '' ? String(data.exit_date) : null;
